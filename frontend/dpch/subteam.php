...
$ts = new STTableStatistics($project->getPrefix() . '_subteamoffset', $datum, $db, $team);
$ts->gather();
...
for($i=0;$i<count($mbs);$i++)
{
	...
	$rmlpage .= '[td][url="' . $baseUrl . '/?prefix=' . $project->getPrefix() . '&amp;mode=detail&amp;tabel=subteamoffset&amp;team=' . rawurlencode($team) . '&amp;datum=' . $datum . '&amp;naam=' . rawurlencode($mbs[$i]->getName()) . '"]' . $mbs[$i]->getName() . '[/url][/td]';
	$rmlpage .= '[td align="right"][blue]' . number_format($mbs[$i]->getCredits(), 0, ',', '.') . '[/blue][/td]';
	$rmlpage .= '[td align="right"](' . $mbs[$i]->getCurrRank() . ')[/td]';
	$rmlpage .= '[/tr]';
}

$rmlpage .= '[tr][td][/td][td][url="' . $baseUrl . '/?prefix=' . $project->getPrefix() . '&amp;datum=' . $datum . '&amp;tabel=subteamoffset&amp;team=' . rawurlencode($team) . '"]More...[/url][/td][/tr]' . "\n\n";
$rmlpage .= '[/table]' . "\n";
...
if ( $project->getSubteamDaily() > 15 )
{
	...
	$ml = new MemberList($project->getPrefix() . '_memberoffset', $datum, ( $project->getSubteamDaily() - 8 ), 15, $db);
	$ml->generateFlushList();
...
if ( $project->getSubteamRank() > 15 )
{
	...
	$ml = new MemberList($project->getPrefix() . '_memberoffset', $datum, ( $project->getSubteamRank() - 8 ), 15, $db);
	$ml->generateRankList();
...
if ( count($mpl) > 0 )
{
	...
	{
		$rmlpage .= '   [tr]';
		$rmlpage .= '    [td][url="' . $baseUrl . '/?mode=detail&amp;tabel=subteamoffset&amp;prefix=' . $project->getPrefix() . '&amp;team=' . rawurlencode($team) . '&amp;naam=' . rawurlencode($mpl[$i]->getName()) . '&amp;datum=' . $datum . '"]' . $mpl[$i]->getName() . '[/url][/td]';
...
$mi = new MemberInfo($db, $team, $project->getPrefix() . '_memberoffset', $datum, $project->getPrefix(), 'memberoffset', $team);
